...
namespace XGTech.Utility
{
	...
	public static class IQueryBuilderExtensions
	{
		...
		/// <param name="q">动态查询条件创建者</param>
		/// <param name="property">属性</param>
        /// <param name="value">查询值 注意:value类型要与比较的类型相同</param>
		/// <returns></returns>
		public static IQueryBuilder<T> Like<T>(this IQueryBuilder<T> q, Expression<Func<T, string>> property, object value)
		{
        ...
        /// <param name="exclude">排除值（意味着如果value==exclude，则当前条件不被包含到查询中）注意:exclude类型要与比较的类型相同</param>
		/// <returns></returns>
		public static IQueryBuilder<T> Equals<T, P>(this IQueryBuilder<T> q, Expression<Func<T, P>> property, object value, object exclude = null)
		{
            if(value==null || string.IsNullOrEmpty(value.ToString()))
            {
                return q;
            ...
            if (exclude != null)
            {
                P tempObj2 = (P)exclude;
                if (tempObj2.Equals(tempObj1))
                {
                    return q;
                }
            }

            return Equals(q, property, tempObj1);
		}
		...
		/// <param name="value">查询值</param>
		/// <returns></returns>
        private static IQueryBuilder<T> Equals<T, P>(this IQueryBuilder<T> q, Expression<Func<T, P>> property, P value)
		{
